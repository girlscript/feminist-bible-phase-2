...
exports.approveRequest = async (req, res) => {
    try{
        ...
        if(org.approved != true && org.declined != true){
            org = await Organization.findByIdAndUpdate({_id: req.params.orgid}, {approved:true}, 
                {
                    new: true,
                    runValidators: true
                })
...
exports.declineRequest = async(req, res)=>{
    try{
        let org = await Organization.findById({_id:req.params.orgid})
        if(org.approved != true && org.declined!=true){
            org = await Organization.findByIdAndUpdate({_id: req.params.orgid}, {declined:true}, 
                {
                    new: true,
                    runValidators: true
                })
